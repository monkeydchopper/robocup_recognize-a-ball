...
int white_points_low_1 = 70;
int white_points_up_1 = 200;
int green_points_low_1 = 5;
int green_points_up_1= 90;
double t1,t2;
...
#ifndef GONG_KONG
		cvCreateTrackbar("white_points_low","Image",&white_points_low_2,300,NULL);		//用于调节白点绿点的阈值
...
#endif
		mat = cvGetMat(white_cvtImage, &temp);  //深拷贝
...
bool get_ball_pos_1(CvMat *src){
	const double area_ratio_limit = 0.6;	//可调
	CvPoint2D32f center;
	float real_radius=0;
	double cont_area;
	double circle_area;
...
//		cout<<"cont_total="<<cont->total<<endl;
		if(real_radius < 5 || real_radius > 150){
			continue;
		}
...
//     cvShowImage("src", pSrcImage);
// 	cvSetMouseCallback("src", on_mouse, (IplImage*)pSrcImage);
